...
#define CS_PIN GPIO_PIN_15 //ORANGE
#define CS_PORT GPIOA

#define SID_PIN GPIO_PIN_12 //RED
...
void ST7920_Animate(const unsigned char* previous,const unsigned char* next)
{
	uint8_t x, y;
	for(y = 0; y < 64; y++)
	{
		if(y < 32)
		{
			for(x = 0; x < 8; x++)							// Draws top half of the screen.
			{												// In extended instruction mode, vertical and horizontal coordinates must be specified before sending data in.
				if(previous[2*x + 16*y]==next[2*x + 16*y] && previous[2*x+1 + 16*y]==next[2*x+1 + 16*y])continue;
				//if(previous[2*x+1 + 16*y]==next[2*x+1 + 16*y])continue;
				ST7920_SendCmd(0x80 | y);				// Vertical coordinate of the screen is specified first. (0-31)
				ST7920_SendCmd(0x80 | x);				// Then horizontal coordinate of the screen is specified. (0-8)
				ST7920_SendData(next[2*x + 16*y]);		// Data to the upper byte is sent to the coordinate.
				ST7920_SendData(next[2*x+1 + 16*y]);	// Data to the lower byte is sent to the coordinate.
			}
		}
		else
		{
			...
			{												// Actions performed as same as the upper half screen.
				...
				ST7920_SendCmd(0x80 | (y-32));			// Vertical coordinate must be scaled back to 0-31 as it is dealing with another half of the screen.
				ST7920_SendCmd(0x88 | x);
				ST7920_SendData(next[2*x + 16*y]);
				ST7920_SendData(next[2*x+1 + 16*y]);
			}
		}
...
void RenderTwoNumbers(const unsigned char* left_num,const unsigned char* right_num,unsigned char* result)
{
	for(int i=0;i<8;i++)
	{
		result[i]=left_num[i];
...
void ST7920_DrawBigNumber(const unsigned char* graphic,uint8_t quarter)
{
	uint8_t x, y;
	for(y = 0; y < 43; y++)
	{
		if(y < 32)
		{
			for(x = 0; x < 2; x++)							// Draws top half of the screen.
			{												// In extended instruction mode, vertical and horizontal coordinates must be specified before sending data in.
				ST7920_SendCmd(0x80 | y);				// Vertical coordinate of the screen is specified first. (0-31)
				ST7920_SendCmd(0x80 | x+(quarter*2));				// Then horizontal coordinate of the screen is specified. (0-8)
				ST7920_SendData(graphic[2*x + 4*y]);		// Data to the upper byte is sent to the coordinate.
				ST7920_SendData(graphic[2*x+1 + 4*y]);	// Data to the lower byte is sent to the coordinate.
			}
		}
		else
		{
			for(x = 0; x < 2; x++)							// Draws bottom half of the screen.
			{												// Actions performed as same as the upper half screen.
				ST7920_SendCmd(0x80 | (y-32));			// Vertical coordinate must be scaled back to 0-31 as it is dealing with another half of the screen.
				ST7920_SendCmd(0x88 | x+(quarter*2));
				ST7920_SendData(graphic[2*x + 4*y]);
				ST7920_SendData(graphic[2*x+1 + 4*y]);
			}
		}
...
//0,43,8,21
void ST7920_DrawBitmap_Any(const unsigned char* graphic,uint8_t x_coord, uint8_t y_coord,uint8_t x_size,uint8_t y_size)// x from 1 to 16
{
	uint8_t x, y, y_s, x_s;
	for(y = 0; y < y_size; y++)
	{
		...
		if(y_s < 32)
		{
			for(x = 0; x < x_size; x++)							// Draws top half of the screen.
			{												// In extended instruction mode, vertical and horizontal coordinates must be specified before sending data in.
				x_s=x+x_coord;
				ST7920_SendCmd(0x80 | (y_s));				// Vertical coordinate of the screen is specified first. (0-31)
				ST7920_SendCmd(0x80 | x_s);				// Then horizontal coordinate of the screen is specified. (0-8)
				ST7920_SendData(graphic[2*x + (x_size*2)*y]);		// Data to the upper byte is sent to the coordinate.
				ST7920_SendData(graphic[2*x+1 + (x_size*2)*y]);	// Data to the lower byte is sent to the coordinate.
			}
		}
		else
		{
			for(x = 0; x < x_size; x++)							// Draws bottom half of the screen.
			{												// Actions performed as same as the upper half screen.
				x_s=x+x_coord;
				ST7920_SendCmd(0x80 | (y_s-32));			// Vertical coordinate must be scaled back to 0-31 as it is dealing with another half of the screen.
				ST7920_SendCmd(0x88 | x_s);
				ST7920_SendData(graphic[2*x + (x_size*2)*y]);
				ST7920_SendData(graphic[2*x+1 + (x_size*2)*y]);
			}
		}
